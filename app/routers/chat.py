...
def _is_quota_error(error: Exception) -> bool:
    ...
    return (
        ...
        or "free tier" in text.lower()
        or (isinstance(error, APIStatusError) and error.status_code == 403)
    )


def _quota_message(model: str) -> str:
    fallback = settings.llm_fallback_model.strip()
    fallback_tip = (
        f"也可以在 .env 里配置 LLM_FALLBACK_MODEL={fallback} 作为备用模型。"
        if fallback
        else "也可以在 .env 里设置 LLM_FALLBACK_MODEL 为另一个可用模型。"
    )
    return (
        f"当前问答模型 {model} 的免费额度已耗尽，DashScope 拒绝了本次调用。\n\n"
        "处理方式：\n"
        "1. 到 DashScope 控制台关闭 “use free tier only / 仅使用免费额度” 模式，允许按量付费；\n"
        "2. 或把 .env 里的 LLM_MODEL 换成当前账号还有额度的模型；\n"
        f"3. {fallback_tip}\n\n"
        "修改 .env 后需要重启后端服务。"
    )


def _chat_completion(client: OpenAI, messages: list[dict], *, stream: bool = False, temperature: float = 0.5):
    ...
    for model in models:
        try:
            ...
            last_error = e
            if _is_quota_error(e) and model != models[-1]:
                logger.warning(f"模型 {model} 配额不可用，尝试备用模型 {models[-1]}")
                continue
            raise

    if last_error:
        raise last_error
    raise RuntimeError("没有可用的 LLM 模型配置")
...
async def _prepare_messages(request: ChatRequest, db: AsyncSession) -> tuple[list[dict], List[dict], str]:
    ...
    logger.info(
        "路由输入: question_len={} folder_ids={} has_data={} is_collection_intent={}",
        len(question),
        folder_ids,
        has_data,
...
async def ask_question(request: ChatRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        client = _get_llm_client()
        response = _chat_completion(client, messages, temperature=0.5)
        return ChatResponse(answer=response.choices[0].message.content or "", sources=sources[:5])
    except HTTPException: raise
    except Exception as e:
        logger.error(f"问答失败: {e}")
        if _is_quota_error(e):
            return ChatResponse(answer=_quota_message(settings.llm_model), sources=[])
...
async def ask_question_stream(request: ChatRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        def generate():
            try:
                stream = _chat_completion(client, messages, temperature=0.5, stream=True)
                for chunk in stream:
                ...
                if _is_quota_error(e):
                    yield _quota_message(settings.llm_model)
                    yield "\n[[SOURCES_JSON]][]"
                    return
                yield f"问答失败: {str(e)}"
                yield "\n[[SOURCES_JSON]][]"
