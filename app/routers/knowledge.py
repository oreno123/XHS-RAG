...
async def _get_session_folder_ids(db: AsyncSession, session_id: str) -> List[int]:
    """获取当前登录用户可访问的内部收藏夹 ID。"""
    result = await db.execute(
    ...
    rows = await db.execute(
        select(FavoriteFolder.id).where(FavoriteFolder.session_id.in_(target_session_ids))
    )
    return [row[0] for row in rows.fetchall()]


async def _delete_vectors_without_other_refs(
    db: AsyncSession,
    rag: RAGService,
    bvids: set[str],
    excluded_folder_ids: List[int],
) -> int:
    """仅删除不再被其他收藏夹引用的视频向量。"""
    deleted = 0
    for bvid in bvids:
        other_count = await db.scalar(
            ...
            .where(
                ...
                FavoriteVideo.folder_id.not_in(excluded_folder_ids),
            )
        )
        if other_count:
            continue
...
def _extract_video_info(media: dict) -> tuple[str, str, Optional[int]]:
    ...
    cid = (
        ugc.get("first_cid")
        or media.get("cid")
        or media.get("first_cid")
        or media.get("page", {}).get("cid")
    )
...
async def clear_knowledge_base(
    ...
    try:
        folder_ids = await _get_session_folder_ids(db, session_id)
        if not folder_ids:
            return {"message": "当前会话没有可清空的知识库内容"}
        ...
        rag = get_rag_service()
        deleted_vectors = await _delete_vectors_without_other_refs(
            db, rag, bvids, folder_ids
        )

        await db.execute(delete(FavoriteVideo).where(FavoriteVideo.folder_id.in_(folder_ids)))
        await db.commit()
        return {
            "message": "当前用户知识库已清空",
...
async def delete_video_from_knowledge(
    ...
    try:
        folder_ids = await _get_session_folder_ids(db, session_id)
        if not folder_ids:
            raise HTTPException(status_code=404, detail="视频不属于当前用户知识库")

        exists = await db.scalar(
            select(func.count())
            .select_from(FavoriteVideo)
            .where(FavoriteVideo.folder_id.in_(folder_ids), FavoriteVideo.bvid == bvid)
        )
        if not exists:
            raise HTTPException(status_code=404, detail="视频不属于当前用户知识库")
        ...
        await db.commit()
        return {"message": f"已删除视频 {bvid}", "deleted_vectors": deleted_vectors}
